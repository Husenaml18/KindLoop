 ...
 * The ten experiences.
 *
 * Everything a gift can be on its own, and everything a Personalized Website can be
 * assembled from. `registry.ts` composes this with Personalized Website itself to make
 * the full set the rest of the app sees; nothing else should import this file.
 */
...
import { redFlagsContentSchema, emptyRedFlagsContent } from "./red-flags/schema";
...
export const SECTION_REGISTRY = {
  ...
  "my-red-flags": {
    id: "my-red-flags",
    displayName: "My Red Flags (That I'm Working On)",
    description:
      "A notebook of the habits you're actually working on, one chapter each — the flag, where it comes from, what you're doing about it, and a small win. Not an apology. A plant grows as they read.",
    isPaid: false,
    contentSchema: redFlagsContentSchema,
    emptyContent: emptyRedFlagsContent,
    Editor: RedFlagsEditor,
    View: RedFlagsView,
    fullWidthEditor: true,
    hasDemo: true,
  },
...
export type SectionTemplateId = keyof typeof SECTION_REGISTRY;

/**
 * Look up one of the ten experiences.
 *
 * This is what Personalized Website composes from, and the reason it is a separate
 * module from `registry.ts`: a website is itself a template, so if it read the
 * full registry it could contain another website, and that one another — an
 * infinite regress that no amount of UI guarding fixes properly. Here the
 * impossibility is structural. It also breaks what would otherwise be an import
 * cycle between the registry and the website that reads it.
 */
export function getSectionTemplate(
  id: string
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- see above
): TemplateDefinition<any> | undefined {
  return SECTION_REGISTRY[id as SectionTemplateId];
}
